...
module.exports = {
        ...
        pool.getConnection(function(err, con) {
            con.query("SELECT sid FROM route WHERE rider_id='"+JSON.stringify(body.sid)+"' AND pickup_time IS NULL;", (err,rows) => {
                ...
                if (rows.length > 0){
                    ...
                    /*con.query('INSERT INTO user SET ?', body, (err, response) => {
                        if(err) throw err;
                        json.msg = "rider succesfully activated";
                        result(json);
                    });*/
                    con.release((err) => {
                    });
    ...
    //Cancel a carpool journey
    cancelPool(body, result) {
        var json = {};
        pool.getConnection(function(err, con) {
            con.query("DELETE FROM route WHERE route_id='"+body.route_id+"' AND driverid='"+body.driverid+"';", (err, row) => {
                if(err) throw err;
                json.msg = "pool successfully cancelled";
                result(json);
                con.release((err) => {
                });
            });
